...
# convert all values to an integer numbers
#interval$steps <- as.integer(interval$steps)
...
I will replace NA values with the mean number of steps per same 5-minute interval
```{r}
# use data frame, prepared on previous step
# round values to integer
interval$steps <- round(interval$steps, 0)
```

3. Create a new dataset with the missing data filled in
```{r}
# replace NA values with mean number of steps per same 5-minute interval
for (i in 1:nrow(df)) {
    if(is.na(df$steps[i]))
       df$steps[i] <- interval$steps[df$interval[i] %in% interval$interval]
}
```
